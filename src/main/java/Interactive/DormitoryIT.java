...
public class DormitoryIT {
	 ...
	 * transaction.rollback(); } } catch(NullPointerException np) { } finally {
	 * sessionObj.close(); }
	 * 
	 * if (sessionFactoryObj != null) { sessionFactoryObj.close(); } }
	 * 
	 * public Dormitory searchDorm(String dorm_name) { try { sessionFactoryObj = new
	 * Configuration().configure().buildSessionFactory(); } catch (Throwable ex) {
	 * ex.printStackTrace(); }
	 * 
	 * Session sessionObj = sessionFactoryObj.openSession(); Dormitory dorm = null;
	 * try { dorm = (Dormitory) sessionObj.get(Dormitory.class.getName(),
	 * dorm_name);
	 * 
	 * } catch(HibernateException hibernateExeption) {
	 * System.out.println("error\n"); }finally { sessionObj.close(); }
	 * 
	 * if (sessionFactoryObj != null) { sessionFactoryObj.close(); } return dorm; }
	 */
}
